...
class ParticleEmitter(Entity):
    ...
    def __init__(self, position=Vec3(0,0,0), file='particles/snowsplash.ptf', life=1, deathtime=2, parent=None, force=None):
        ...
        if parent == None:
            ...
            self.effect.start(parent=parent, renderParent=parent)
...
if __name__ == "__main__":    
    ...
    def create_explosion(position):
        ...
        fire = ParticleEmitter(position=position, file=r'effects\fire_sphere.ptf')
        fire.scale_z = 2
        fire.rotation_y = 180
        fire.rotation_x = -11
        fire.y += 0.7
        fire.z -= 0.7
        smoke = ParticleEmitter(parent=cannon, position=position, file=r'effects\smoke.ptf')
        smoke.z -= 1
        smoke.scale=3
        return
